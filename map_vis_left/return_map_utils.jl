...
function generate_ics!(ics_probs, p, eq, saddle, start, stop, res)

    xs = range(eq[1] + start*(saddle[1]-eq[1]), saddle[1] - (1-stop)*(saddle[1]-eq[1]), length = res)
    cas = range(eq[5] + start*(saddle[5]-eq[5]), saddle[5] - (1-stop)*(saddle[5]-eq[5]), length = res)
    ns = range(eq[3] + start*(saddle[3]-eq[3]), saddle[3] - (1-stop)*(saddle[3]-eq[3]), length = res)
    hs = range(eq[4] + start*(saddle[4]-eq[4]), saddle[4] - (1-stop)*(saddle[4]-eq[4]), length = res)
    vs = range(eq[6] + start*(saddle[6]-eq[6]), saddle[6] - (1-stop)*(saddle[6]-eq[6]), length = res)
    ics = Vector{SVector{6,Float64}}(undef, res)
    Threads.@threads for i=1:length(cas)
        ics[i] = @SVector [xs[i], 0.0, ns[i], hs[i], cas[i], vs[i]]
    end
    return ics
end
...
function get_saddle_traj(prob,p)
    ...
    upper_prob = remake(prob, u0 = upper_u0, p = Params(p, prob.p.eq, saddle, false))
    upper_sol = solve(upper_prob, RK4(), abstol = 1e-8, reltol = 1e-8, callback = cb)
    lower_prob = remake(prob, u0 = lower_u0, p = Params(p, prob.p.eq, saddle, false))
